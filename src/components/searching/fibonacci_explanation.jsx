...
import { FiCode, FiCopy, FiShare, FiShare2, FiInfo, FiClock, FiLayers, FiTarget, FiCpu, FiLightbulb } from 'react-icons/fi';
import { BsFillLightningFill, BsGraphUp } from 'react-icons/bs';
import { IoMdSchool } from 'react-icons/io';
import { FaRegLightbulb, FaSort } from 'react-icons/fa';


const FibonacciSearchExplanation = () => {
    const [activeTab, setActiveTab] = useState('details');
    const [isHovered, setIsHovered] = useState(false);

    // Function to copy pseudocode to clipboard
    const handleCopy = () => {
        const pseudocode = `fibonacciSearch(arr, target):
    ...
    const applications = [
        { title: 'Search in Sorted Arrays', description: 'Quickly locates elements in sorted datasets without division.', icon: <FiTarget size={20} className="text-pink-400" /> },
        { title: 'Optimized Database Queries', description: 'Improve lookup speeds by minimizing comparisons in databases.', icon: <BsGraphUp size={20} className="text-blue-400" /> },
        { title: 'Systems with Limited Hardware', description: 'Ideal for hardware lacking efficient division operations.', icon: <FiLayers size={20} className="text-purple-400" /> },
        { title: 'Data Compression', description: 'Enable algorithms', icon: <FiCpu size={20} className="text-green-400" /> },
    ];

    // Learning resources
    const resources = [
        { title: 'Interactive Fibonacci Search Visualization', url: 'https://www.example.com/fibonacci-search-visualization', description: 'See Fibonacci Search in action with step-by-step visualization' },
        { title: 'Common Fibonacci Search Interview Questions', url: 'https://www.example.com/fibonacci-search-interview-questions', description: 'Practice with frequently asked interview problems' },
        { title: 'Fibonacci Search vs Binary Search Comparison', url: 'https://www.example.com/fibonacci-search-vs-binary-search', description: 'Understand when to choose Fibonacci Search over Binary Search' },
    ];
                ...
                >
                    ...
                    >
                        <p className="text-gray-200 text-lg md:text-xl leading-relaxed text-center max-w-3xl mx-auto font-light">
                            Fibonacci Search is a <span className="text-purple-300 font-semibold">comparison-based technique</span> that uses <span className="text-indigo-300 font-semibold">Fibonacci numbers</span> to efficiently search a sorted array, avoiding division operations for uniform data.
                        </p>
                        ...
                        <AnimatePresence>
                            {isHovered && (
                                ...
                                >
                                    ...
                                    It&apos;s useful for data compression.
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ul className="space-y-5">
                                        {[
                                            { icon: <FiClock size={20} />, label: 'Time Complexity', value: 'O(log n)', color: 'text-purple-400', tooltip: 'Logarithmic time using Fibonacci sequence' },
                                            { icon: <FiLayers size={20} />, label: 'Space Complexity', value: 'O(1)', color: 'text-indigo-400', tooltip: 'Constant space, no extra memory' },
                                            { icon: <FiTarget size={20} />, label: 'Best for', value: 'Uniform data, division-free search', color: 'text-blue-400', tooltip: 'Great for uniform arrays and hardware lacking efficient division' },
                                            { icon: <FiCpu size={20} />, label: 'Data Use', value: 'Data compression', color: 'text-green-400', tooltip: 'Used on algorithms of data compression' },
                                            { icon: <FaSort size={20} />, label: 'Prerequisite', value: 'Sorted array', color: 'text-yellow-400', tooltip: 'Requires pre-sorted data' },
                                ...
                                >
                                    ...
                                    <ul className="space-y-4">
                                        {[
                                            "Explores a sorted array using Fibonacci numbers",
                                    ...
                                    >
                                        <p className="text-indigo-300 font-medium flex items-center">
                                            ...
                                            Fibonacci search avoids division operations, which is best suited for uniform data
                                        </p>
                                    </motion.div>
                                </motion.div>
                            </motion.div>
                                ...
                                <motion.div variants={itemVariants} className="bg-gray-800/70 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-purple-500/30">
                                    ...
                                    <pre className="font-mono text-sm md:text-base text-gray-200 whitespace-pre-wrap bg-gray-700/30 p-4 rounded-lg overflow-x-auto">
                                        <code className="block text-purple-200">
                                            ...
                                            <span className="ml-4 text-indigo-300">fibM2 = 0, fibM1 = 1, fibM = fibM1 + fibM2</span>{'\n'}
                                            <span className="ml-4 text-indigo-300">offset = -1</span>{'\n'}
                                            <span className="ml-4 text-blue-300">{"while fibM < arr.length:"}</span>{'\n'}
                                            <span className="ml-8 text-green-300">fibM2 = fibM1</span>{'\n'}
                                            <span className="ml-8 text-green-300">fibM1 = fibM</span>{'\n'}
                                            <span className="ml-8 text-green-300">fibM = fibM1 + fibM2</span>{'\n'}
                                            <span className="ml-4 text-blue-300">{"while fibM > 1:"}</span>{'\n'}
                                            <span className="ml-8 text-yellow-300">i = min(offset + fibM2, arr.length - 1)</span>{'\n'}
                                            <span className="ml-8 text-blue-300">if arr[i] == target:</span>{'\n'}
                                            <span className="ml-12 text-green-300">return i</span>{'\n'}
                                            <span className="ml-8 text-blue-300">{"if arr[i] < target:"}</span>{'\n'}
                                            <span className="ml-12 text-green-300">offset = i</span>{'\n'}
                                            <span className="ml-12 text-green-300">fibM = fibM1</span>{'\n'}
                                            <span className="ml-12 text-green-300">fibM1 = fibM2</span>{'\n'}
                                            <span className="ml-12 text-green-300">fibM2 = fibM - fibM1</span>{'\n'}
                                            <span className="ml-8 text-blue-300">else:</span>{'\n'}
                                            <span className="ml-12 text-green-300">fibM = fibM2</span>{'\n'}
                                            <span className="ml-12 text-green-300">fibM1 = fibM1 - fibM2</span>{'\n'}
                                            <span className="ml-12 text-green-300">fibM2 = fibM - fibM1</span>{'\n'}
                                            <span className="ml-4 text-blue-300">if fibM1 == 1 and arr[offset + 1] == target:</span>{'\n'}
                                            <span className="ml-8 text-green-300">return offset + 1</span>{'\n'}
                                            <span className="ml-4 text-yellow-300">return -1</span>
                                        </code>
                                    </pre>
                                </motion.div>
                                    ...
                                    {applications.map((app, idx) => (
                                        ...
                                        >
                                            <div className="flex items-center mb-3">
                                                ...
                                                <p className="ml-1"> {app.description}</p>
                                            </div>
...
export default FibonacciSearchExplanation;