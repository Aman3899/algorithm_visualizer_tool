...
import Tippy from '@tippyjs/react'; // For tooltips
import 'tippy.js/dist/tippy.css'; // Tooltip styles
import 'tippy.js/themes/light-border.css'; // Optional: light-border theme
import { FiCopy, FiShare, FiShare2, FiInfo, FiClock, FiLayers, FiTarget, FiCpu, FiLightbulb, FiCode } from 'react-icons/fi';
...
const BinarySearchExplanation = () => {
    ...
    const applications = [
        { title: 'Searching in Databases', description: 'Efficiently retrieves data from large databases with sorted indices.', icon: <FiTarget size={20} className="text-pink-400" /> },
        { title: 'Dictionaries and Lexicons', description: 'Looks up words swiftly in alphabetized dictionaries.', icon: <BsGraphUp size={20} className="text-blue-400" /> },
        { title: 'Code Debugging', description: 'Finds error-causing values in a sorted log or debug output.', icon: <FiLayers size={20} className="text-purple-400" /> },
        { title: 'Data Compression', description: 'Enables efficient data retrieval', icon: <FiCpu size={20} className="text-green-400" /> },
    ];

    const resources = [
        { title: 'Interactive Binary Search Visualization', url: 'https://www.example.com/binary-search-visualization', description: 'See Binary Search in action with step-by-step visualization' },
        { title: 'Common Binary Search Interview Questions', url: 'https://www.example.com/binary-search-interview-questions', description: 'Practice with frequently asked interview problems' },
        { title: 'Binary Search vs Linear Search Comparison', url: 'https://www.example.com/binary-search-vs-linear-search', description: 'Understand when to choose Binary Search over Linear Search' },
    ];
                ...
                >
                    ...
                    >
                        <p className="text-gray-200 text-lg md:text-xl leading-relaxed text-center max-w-3xl mx-auto font-light">
                            Binary Search is an <span className="text-purple-300 font-semibold">efficient algorithm</span> that finds a target value in a <span className="text-indigo-300 font-semibold">sorted array</span>. It halves the search space with each step, zeroing in on the target with precision.
                        </p>
                        ...
                        <AnimatePresence>
                            {isHovered && (
                                ...
                                >
                                    ...
                                    Halving each search, and for that needs to have a Sorted Array
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ul className="text-gray-200 list-none space-y-3">
                                        {[
                                            { label: 'Time Complexity', value: 'O(log n)', color: 'text-purple-400', tooltip: 'Logarithmic time due to halving the search space' },
                                            { label: 'Space Complexity', value: 'O(1) iterative, O(log n) recursive', color: 'text-indigo-400', tooltip: 'Minimal space, recursive uses call stack' },
                                            { label: 'Prerequisite', value: 'Array must be sorted', color: 'text-yellow-400', tooltip: 'Requires pre-sorted data for correctness' },
                                            { label: 'Best Use', value: 'Static sorted datasets', color: 'text-blue-400', tooltip: 'Ideal for unchanging, ordered lists' },
                                        ].map((item, idx) => (
