...
const JumpSearchExplanation = () => {
    const [activeTab, setActiveTab] = useState('details');
    const [isHovered, setIsHovered] = useState(false);

    // Function to copy pseudocode to clipboard
    const handleCopy = () => {
        const pseudocode = `jumpSearch(arr, target):
    n = arr.length
    step = √n
    prev = 0
    ...
    const applications = [
        { title: 'Large Databases', description: 'Efficient for searching large, sorted datasets with fewer jumps.', icon: <FiTarget size={20} className="text-pink-400" /> },
        { title: 'Search in Indexes', description: 'Enhances the speed of queries by reducing memory access points.', icon: <BsGraphUp size={20} className="text-blue-400" /> },
        { title: 'Sparse Array Search', description: 'Optimizes searches in arrays with widely spread-out data.', icon: <FiLayers size={20} className="text-purple-400" /> },
        { title: 'Array Compression', description: 'Supports data retrieval where size has been a key', icon: <FiCpu size={20} className="text-green-400" /> },
    ];

    const resources = [
        { title: 'Interactive Jump Search Visualization', url: 'https://www.example.com/jump-search-visualization', description: 'See Jump Search in action with step-by-step visualization' },
        { title: 'Common Jump Search Interview Questions', url: 'https://www.example.com/jump-search-interview-questions', description: 'Practice with frequently asked interview problems' },
        { title: 'Jump Search vs Linear Search Comparison', url: 'https://www.example.com/jump-search-vs-linear-search', description: 'Understand when to choose Jump Search over Linear Search' },
    ];
                ...
                >
                    ...
                    >
                        <p className="text-gray-200 text-lg md:text-xl leading-relaxed text-center max-w-3xl mx-auto font-light">
                            Jump Search is an algorithm for <span className="text-purple-300 font-semibold">sorted arrays</span> that jumps ahead by fixed steps (typically <span className="text-indigo-300 font-semibold">√n</span>) to find the block containing the target, then performs a linear search within that block. It balances the efficiency of binary search with simplicity.
                        </p>
                        ...
                        <AnimatePresence>
                            {isHovered && (
                                ...
                                >
                                    ...
                                    It performs best on medium-sized datasets, balancing jump and linear search costs.
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ul className="text-gray-200 list-none space-y-3">
                                        {[
                                            { label: 'Time Complexity', value: 'O(√n)', color: 'text-purple-400', tooltip: 'Search time increases by the square root of data size.' },
                                            { label: 'Space Complexity', value: 'O(1)', color: 'text-indigo-400', tooltip: 'Maintains small space in memory, with low stack call.' },
                                            { label: 'Best for', value: 'Medium-sized, sorted arrays', color: 'text-blue-400', tooltip: 'The search method is optimized for middle loads.' },
                                            { label: 'Prerequisite', value: 'Sorted array', color: 'text-yellow-400', tooltip: 'It must be sorted to work correctly.' }
                                        ].map((item, idx) => (
                                ...
                                <motion.div variants={itemVariants} className="bg-gray-800/70 backdrop-blur-sm p-6 rounded-2xl shadow-xl border border-purple-500/30">
                                    ...
                                    <pre className="font-mono text-sm md:text-base text-gray-200 whitespace-pre-wrap bg-gray-700/30 p-4 rounded-lg overflow-x-auto">
                                        <code className="block text-purple-200">
                                            ...
                                            <span className="ml-4 text-indigo-300">step = √n</span>{'\n'}
                                            <span className="ml-4 text-indigo-300">prev = 0</span>{'\n'}
                                            <span className="ml-4 text-blue-300">{"while arr[min(step, n)-1] < target:"}</span>{'\n'}
                                            <span className="ml-8 text-yellow-300">prev = step</span>{'\n'}
                                            <span className="ml-8 text-yellow-300">step += √n</span>{'\n'}
                                            <span className="ml-8 text-blue-300">{"if prev >= n:"}</span>{'\n'}
                                            <span className="ml-12 text-green-300">return -1</span>{'\n'}
                                            <span className="ml-4 text-blue-300">for i = prev to min(step, n)-1:</span>{'\n'}
...
export default JumpSearchExplanation;