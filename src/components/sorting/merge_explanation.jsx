...
const MergeSortExplanation = () => {
    ...
    const pseudocode = `mergeSort(arr, start, end):
    if start >= end:
        return [arr[start]]
...
merge(left, right):
    ...
    const applications = [
        { title: 'Large Datasets', description: 'Efficiently sorts large arrays with predictable performance.', icon: <FiTarget size={20} className="text-pink-400" /> },
        { title: 'External Sorting', description: 'Used in disk-based sorting when data doesn’t fit in memory.', icon: <BsGraphUp size={20} className="text-blue-400" /> },
        { title: 'Stable Sorting', description: 'Maintains relative order of equal elements in sorted output.', icon: <FiLayers size={20} className="text-purple-400" /> },
        { title: 'Parallel Processing', description: 'Divide-and-conquer nature suits multi-threaded implementations.', icon: <FiCpu size={20} className="text-green-400" /> },
    ];

    // Learning resources
    const resources = [
        { title: 'Interactive Merge Sort Visualization', url: 'https://www.example.com/merge-sort-visualization', description: 'See Merge Sort in action with step-by-step visualization.' },
        { title: 'Merge Sort Interview Questions', url: 'https://www.example.com/merge-sort-questions', description: 'Practice with common interview problems.' },
        { title: 'Merge Sort vs Other Sorts', url: 'https://www.example.com/merge-vs-others', description: 'Compare Merge Sort with Quick Sort, Heap Sort, etc.' },
    ];
                ...
                >
                    ...
                    >
                        <p className="text-gray-200 text-lg md:text-xl leading-relaxed text-center max-w-3xl mx-auto font-light">
                            Merge Sort uses a <span className="text-purple-300 font-semibold">divide-and-conquer</span> approach, splitting the array into halves recursively until single elements remain, then <span className="text-indigo-300 font-semibold">merging</span> them back in sorted order.
                        </p>
                        ...
                        <AnimatePresence>
                            {isHovered && (
                                ...
                                >
                                    ...
                                    Stable and predictable, but requires extra space for merging
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.div>
                                ...
                                >
                                    ...
                                    <ul className="space-y-5">
                                        {[
                                            ...
                                            { icon: <FiCpu size={20} />, label: 'Stability', value: 'Stable', color: 'text-blue-400' },
