...
const CountingSortExplanation = () => {
    ...
    const pseudocode = `countingSort(arr, maxValue):
    n = arr.length
    // Initialize count array for values 0 to maxValue
    ...
    const applications = [
        { title: 'Integer Sorting', description: 'Sorts arrays of integers with a small range efficiently.', icon: <FiTarget size={20} className="text-pink-400" /> },
        { title: 'Radix Sort Helper', description: 'Used as a subroutine in Radix Sort for digit-wise sorting.', icon: <BsGraphUp size={20} className="text-blue-400" /> },
        { title: 'Database Indexing', description: 'Organizes records with limited key ranges in databases.', icon: <FiLayers size={20} className="text-purple-400" /> },
        { title: 'Stable Sorting', description: 'Maintains relative order of equal elements in sorting tasks.', icon: <FiCpu size={20} className="text-green-400" /> },
    ];

    // Learning resources
    const resources = [
        { title: 'Interactive Counting Sort Visualization', url: 'https://www.example.com/counting-sort-visualization', description: 'See Counting Sort in action with step-by-step visualization' },
        { title: 'Counting Sort Interview Questions', url: 'https://www.example.com/counting-sort-questions', description: 'Practice with common interview problems' },
        { title: 'Counting Sort vs Other Algorithms', url: 'https://www.example.com/counting-vs-others', description: 'Compare Counting Sort with Bucket Sort, Radix Sort, etc.' },
    ];
                ...
                >
                    ...
                    >
                        <p className="text-gray-200 text-lg md:text-xl leading-relaxed text-center max-w-3xl mx-auto font-light">
                            Counting Sort is a <span className="text-purple-300 font-semibold">non-comparative</span> algorithm that counts occurrences of each value and places them in sorted order, excelling when the range (<span className="text-indigo-300 font-semibold">k</span>) is close to the number of elements (<span className="text-indigo-300 font-semibold">n</span>).
                        </p>
                        ...
                        <AnimatePresence>
                            {isHovered && (
                                ...
                                >
                                    ...
                                    Counting Sort is linear when k ≈ n, making it faster than comparison-based sorts
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ul className="space-y-5">
                                        {[
                                            { icon: <FiClock size={20} />, label: 'Time Complexity', value: 'O(n + k)', color: 'text-purple-400' },
                                            { icon: <FiLayers size={20} />, label: 'Space Complexity', value: 'O(n + k)', color: 'text-indigo-400' },
                                            { icon: <FiCpu size={20} />, label: 'Stability', value: 'Yes', color: 'text-blue-400' },
                                            { icon: <FiTarget size={20} />, label: 'Best for', value: 'Small k', color: 'text-green-400' },
                                        ].map((item, idx) => (
                                            <motion.li
                                                key={idx}
                                                variants={itemVariants}
...
export default CountingSortExplanation;