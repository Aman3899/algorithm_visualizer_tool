...
const HeapSortExplanation = ({ isMaxHeap }) => {
...
heapify(arr, n, i):
    ...
    left = 2 * i + 1
    right = 2 * i + 2
    if left < n and (${isMaxHeap ? 'arr[left] > arr[largestOrSmallest]' : 'arr[left] < arr[largestOrSmallest]'}):
        largestOrSmallest = left
    if right < n and (${isMaxHeap ? 'arr[right] > arr[largestOrSmallest]' : 'arr[right] < arr[largestOrSmallest]'}):
    ...
    const applications = [
        { title: 'Priority Queues', description: 'Efficiently manages tasks based on priority in scheduling systems.', icon: <FiTarget size={20} className="text-pink-400" /> },
        { title: 'Graph Algorithms', description: 'Used in Dijkstra’s algorithm for shortest path computations.', icon: <BsGraphUp size={20} className="text-blue-400" /> },
        { title: 'Order Statistics', description: 'Quickly finds the k-th largest/smallest element in an array.', icon: <FiLayers size={20} className="text-purple-400" /> },
        { title: 'Heap-based Sorting', description: 'Sorts data efficiently in scenarios requiring in-place sorting.', icon: <FiCpu size={20} className="text-green-400" /> },
    ];

    // Learning resources
    const resources = [
        { title: 'Interactive Heap Sort Visualization', url: 'https://www.example.com/heap-sort-visualization', description: 'See Heap Sort in action with step-by-step visualization' },
        { title: 'Heap Sort Interview Questions', url: 'https://www.example.com/heap-sort-questions', description: 'Practice with common interview problems' },
        { title: 'Heap vs Other Sorting Algorithms', url: 'https://www.example.com/heap-vs-others', description: 'Compare Heap Sort with Quick Sort, Merge Sort, etc.' },
    ];
                ...
                >
                    ...
                    >
                        <p className="text-gray-200 text-lg md:text-xl leading-relaxed text-center max-w-3xl mx-auto font-light">
                            Heap Sort uses a <span className="text-purple-300 font-semibold">binary heap</span> to sort an array by building a <span className="text-indigo-300 font-semibold">{isMaxHeap ? 'max' : 'min'} heap</span> and repeatedly extracting the {isMaxHeap ? 'maximum' : 'minimum'} element.
                        </p>
                        ...
                        <AnimatePresence>
                            {isHovered && (
                                ...
                                >
                                    ...
                                    Heap Sort is in-place but not stable, making it efficient for memory-constrained systems
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ul className="space-y-5">
                                        {[
                                            { icon: <FiClock size={20} />, label: 'Time Complexity', value: 'O(n log n)', color: 'text-purple-400' },
                                            { icon: <FiLayers size={20} />, label: 'Space Complexity', value: 'O(1)', color: 'text-indigo-400' },
                                            { icon: <FiCpu size={20} />, label: 'Data Structure', value: 'Heap', color: 'text-blue-400' },
                                            { icon: <FiTarget size={20} />, label: 'Stability', value: 'No', color: 'text-green-400' },
                                        ].map((item, idx) => (
                                            <motion.li
                                                key={idx}
                                                variants={itemVariants}
                                ...
                                >
                                    ...
                                    <ul className="space-y-4">
                                        {[
                                            `Builds a ${isMaxHeap ? 'max' : 'min'} heap to prioritize ${isMaxHeap ? 'largest' : 'smallest'} elements`,
