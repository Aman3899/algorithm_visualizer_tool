...
const InsertionSortVisualizer = () => {
    ...
    const [activeIndex, setActiveIndex] = useState(-1); // Tracks element being inserted
    ...
    const insertionSort = async () => {
        ...
        for (let i = 1; i < tempArray.length && sortingRef.current; i++) {
            let key = tempArray[i];
            let j = i - 1;
            setActiveIndex(i);

            while (j >= 0 && tempArray[j] > key && sortingRef.current) {
    ...
    const getBarColor = (index) => {
        ...
        if (index === activeIndex) {
            return 'bg-gradient-to-t from-yellow-600 to-yellow-400 shadow-yellow-500/40'; // Color for the active element being inserted
        }
                    ...
                    <div className="bg-gray-700/70 rounded-3xl p-4 sm:p-6 h-60 sm:h-96 flex items-end justify-center gap-1 sm:gap-2 overflow-x-auto border border-indigo-600/40">
                        {array.map((value, index) => (
                            <motion.div
                                ...
                                animate={{
                                    y: currentIndices.includes(index) ? -15 : (index === activeIndex ? -15 : 0),
                                    rotateX: currentIndices.includes(index) ? 10 : (index === activeIndex ? 10 : 0),
                                    scale: currentIndices.includes(index) ? 1.05 : (index === activeIndex ? 1.05 : 1),
