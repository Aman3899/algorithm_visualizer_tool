...
const HeapSortVisualizer = () => {
    ...
    const [currentIndices, setCurrentIndices] = useState([]); // Indices being heapified/swapped
    const [heapTree, setHeapTree] = useState([]); // Tree structure for visualization
    const [isMaxHeap, setIsMaxHeap] = useState(true); // Toggle between Max and Min Heap
    ...
    const heapify = async (arr, n, i) => {
        ...
        if (left < n) {
            ...
            if ((isMaxHeap && arr[left] > arr[largestOrSmallest]) || (!isMaxHeap && arr[left] < arr[largestOrSmallest])) {
        ...
        if (right < n) {
            setComparisons(prev => prev + 1);
            if ((isMaxHeap && arr[right] > arr[largestOrSmallest]) || (!isMaxHeap && arr[right] < arr[largestOrSmallest])) {
    ...
    const updateHeapTree = (arr) => {
        ...
        for (let i = 0; i < arr.length; i++) {
            if (nodesInLevel >= Math.pow(2, level)) {
                level++;
                nodesInLevel = 0;
            }
                ...
                >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
                        ...
                        <motion.button
                            whileHover={{ scale: 1.05, boxShadow: '0 0 15px rgba(255,255,255,0.2)' }}
                            whileTap={{ scale: 0.95 }}
                            onClick={() => !sorting && setIsMaxHeap(!isMaxHeap)}
                            disabled={sorting}
                                        ...
                                        <div className="flex gap-4 flex-wrap justify-center">
                                            {level.map((value, nodeIdx) => (
                                                <motion.div
                                                    ...
                                                    animate={{ scale: currentIndices.includes(heapTree.flat().indexOf(value)) ? 1.1 : 1 }}
                                                    transition={{ duration: 0.3 }}
                                                >
                                                    {value}
                                                </motion.div>
                    ...
                    >
                        <h2 className="text-2xl font-bold mb-4 text-indigo-400">Heap Sort Explained</h2>
                        <p className="text-gray-300 mb-4">
                            Heap Sort uses a binary heap data structure. It first builds a {isMaxHeap ? 'max' : 'min'} heap, then repeatedly extracts the {isMaxHeap ? 'maximum' : 'minimum'} element and rebuilds the heap.
                        </p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <ul className="text-gray-300 list-disc list-inside">
                                    ...
                                    <li>Stable: No</li>
                                    <li>Heap Type: {isMaxHeap ? 'Max Heap' : 'Min Heap'}</li>
                                </ul>
                            </div>
                            <div>
                                <h3 className="text-lg font-semibold text-indigo-300 mb-2">Steps</h3>
                                <ol className="text-gray-300 list-decimal list-inside">
                                    <li>Build a {isMaxHeap ? 'max' : 'min'} heap from the array</li>
                                    <li>Extract the root ({isMaxHeap ? 'max' : 'min'} element) and place it at the end</li>
