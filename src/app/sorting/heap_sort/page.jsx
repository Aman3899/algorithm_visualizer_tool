...
const HeapSortVisualizer = () => {
    ...
    // Generate random array (5-100 range for better visualization)
        ...
        try {
            const parsedArray = customInput
                ...
                .map(item => {
                    ...
                    if (isNaN(num) || num < 0 || num > 999) throw new Error(`"${item}" must be between 0 and 999`);
    ...
    const heapSort = async () => {
        ...
        if (sortingRef.current) {
            // For Min Heap, reverse the array to get ascending order (optional)
            if (!isMaxHeap) {
                tempArray.reverse();
                setArray([...tempArray]);
    ...
    const updateHeapTree = (arr) => {
        ...
        for (let i = 0; i < heapSize; i++) {
            ...
            tree[level].push({ value: arr[i], index: i });
        }
        setHeapTree(tree);
    };
    ...
    const getBarColor = (index) => {
        ...
        if (currentIndices.includes(index)) return 'bg-gradient-to-t from-cyan-600 to-cyan-400 shadow-cyan-500/40';
        ...
        <>
            ...
            <div className="min-h-screen bg-gradient-to-br from-gray-900 via-indigo-950 to-black text-white px-4 py-24 font-sans">
                <motion.div
                    ...
                    className="max-w-7xl mx-auto bg-gray-800/90 rounded-3xl shadow-2xl p-8 border border-indigo-600/40 overflow-hidden"
                >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
                        {[
                            ...
                            { icon: null, text: isMaxHeap ? 'Max Heap' : 'Min Heap', onClick: handleHeapToggle, disabled: sorting, bg: 'bg-indigo-600' },
                        ].map((btn, idx) => (
                            <motion.button
                                key={idx}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div>
                            <label className="flex items-center gap-2 text-lg font-semibold text-indigo-300">
                                <FaChartBar /> Size
                            </label>
                            <input
                                type="range"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                        {[
                            ...
                            <motion.div
                                key={idx}
                                className="bg-gray-700/70 p-4 rounded-xl border border-indigo-600/20 shadow-lg"
                    ...
                    <div className="bg-gray-700/70 rounded-3xl p-6 h-96 flex items-end justify-center gap-1 overflow-x-auto border border-indigo-600/30">
                        {array.map((value, index) => (
                            <motion.div
                                key={index}
                                className={`${getBarColor(index)} rounded-t-xl relative shadow-lg`}
                                    ...
                                    <div key={levelIdx} className="flex flex-col items-center">
                                        ...
                                        <div className="flex gap-6 flex-wrap justify-center">
                                            {level.map((node, nodeIdx) => (
                                                <motion.div
                                                    key={nodeIdx}
                                                    className={`${currentIndices.includes(node.index)
                                                            ? 'bg-gradient-to-r from-cyan-600 to-cyan-400'
                                                            : 'bg-gradient-to-r from-indigo-600 to-indigo-400'
                                                        } p-3 rounded-full text-white text-sm font-mono shadow-lg relative`}
                                                    animate={{ scale: currentIndices.includes(node.index) ? 1.1 : 1 }}
                                                    transition={{ duration: 0.3 }}
                                                >
                                                    {node.value}
                                                    {levelIdx > 0 && (
                                                        <div className="absolute -top-8 left-1/2 w-px h-8 bg-gray-400 transform -translate-x-1/2" />
                                                    )}
                                                </motion.div>
                                            ))}
                                        </div>
                    ...
                    <AnimatePresence>
                        {showInputModal && (
                            ...
                            >
                                <motion.div
                                    className="bg-gray-800 rounded-3xl p-6 w-full max-w-md border border-indigo-600/40 shadow-xl"
                                ...
                                >
                                    ...
                                    <p className="text-gray-400 text-sm mt-2">Enter numbers 0-999 (max 31) separated by commas or spaces.</p>
